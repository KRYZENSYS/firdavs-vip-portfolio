  ...
  return (
    <motion.header initial={{ y: -50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.7, delay: 0.2 }}
      ...
      <div className="container-x flex h-16 items-center justify-between md:h-20">
        ...
        <nav className="hidden xl:flex items-center gap-1 max-w-[680px] overflow-x-auto scrollbar-thin">
          {NAV_LINKS.map((l) => (
            <a key={l.href} href={l.href} className="group relative px-2.5 py-2 text-[11px] uppercase tracking-widest text-white/70 hover:text-white transition whitespace-nowrap">
              <span className="relative z-10">{l.label}</span>
              <span className="absolute inset-x-2 -bottom-0.5 h-px scale-x-0 bg-gradient-to-r from-primary to-secondary transition-transform duration-500 group-hover:scale-x-100" />
            </a>
          ))}
        </nav>
        <div className="hidden xl:flex items-center gap-2 shrink-0">
          <a href="#contact" className="rounded-full border border-primary/40 bg-primary/5 px-4 py-1.5 text-xs font-medium text-primary shadow-neon-cyan hover:bg-primary/10 transition">Hire Me</a>
          <ThemeToggle />
          <LangSwitcher />
        </div>
      ...
      {open && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="xl:hidden border-t border-white/10 bg-bg/90 backdrop-blur-xl">
          <div className="container-x flex flex-col py-4 max-h-[80vh] overflow-y-auto">
            {NAV_LINKS.map((l) => (
